...
public class RNNordicDfuModule extends ReactContextBaseJavaModule implements LifecycleEventListener {
    ...
    @ReactMethod
    public void startDFU(String address, String name, String filePath, ReadableMap options, boolean alternativeAdvertisingNameEnabled, Promise promise) {
        mPromise = promise;
        final DfuServiceInitiator starter = new DfuServiceInitiator(address)
                .setKeepBond(false);

        if (options != null && options.hasKey("retries")) {
            int retries = options.getInt("retries");
            starter.setNumberOfRetries(retries);
        }
    
        if (options != null && options.hasKey("mtu")) {
